      ...
      <div class="col-md-4 col-12">
        <div :class="['d-flex mt-2', uSearchBy ? '' : 'mb-2']">
          <input v-model="uSearchBy" autocomplete="off"
            placeholder="Search outward by Invoice Number / Dealer / LR Number" type="text"
            class="form-control bg-white  border-radius-0" @keyup.enter="getOutwardEntry">
          <span id="search" role="button" class="input-group-text border-radius-0 btn-primary"
            @click="getOutwardEntry">Search</span>
        </div>
    ...
    <div class="row">
      <div class="col-12 col-lg-12 col-xxl-12 d-flex">
        <div class="card flex-fill">
          <vue-table-component :key="getOutwardEntryList.length" :columns="outwardColumns" :rows="getOutwardEntryList"
            :sort-by-api="true">
            <template #table-row="slotProps">
              <span v-if="slotProps.column.field === 'actions'">
                <div class="d-flex">
                  <button type="button" class="btn btn-primary me-2" @click="editOutwardEntry(slotProps.row)">
                    <vue-feather type="edit-2" />
                  </button>
                  <button type="button" class="btn btn-danger" @click="deleteOutwardEntry(slotProps.row)">
              ...
              <span
                v-if="slotProps.row.outward_entry_item.length && slotProps.column.field === 'sku' && slotProps.row.outward_entry_item">
                {{ slotProps.row.outward_entry_item.map(i => i.sku) }}
              </span>
              <span
                v-if="slotProps.row.outward_entry_item.length && slotProps.column.field === 'stone_name' && slotProps.row.outward_entry_item">
...
export default {
  ...
  methods: {
    ...
    viewInvoice(row) {
      window.open(`/inventory/manage-outward-entry/invoice/${row.id}`, '_blank');
    },

    // Method to generate an invoice
    generateInvoice(row) {
      this.url = `/inventory/manage-outward-entry/invoice/${row.id}/generate`;
